...
<%@ page import="org.example.tecnet.model.Evento, java.text.SimpleDateFormat" %>
<%
  Evento e = (Evento) request.getAttribute("e");
  boolean edit = (e != null && e.getId() != null);

  SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm");
  String fechaInicio = (edit && e.getFechaInicio() != null) ? sdf.format(e.getFechaInicio()) : "";
  String fechaFin    = (edit && e.getFechaFin() != null)    ? sdf.format(e.getFechaFin())    : "";
%>
<html>
<head>
  ...
  <div class="card">
    <form method="post" action="<%=request.getContextPath()%><%= edit ? "/evento/actualizar" : "/evento/guardar" %>">
      ...
      <div class="form-grid">
        <div class="full">
          <label>Título</label>
          <input class="input" name="titulo" value="<%= edit ? e.getTitulo() : "" %>" required>
        </div>

        <div class="full">
          <label>Descripción</label>
          <input class="input" name="descripcion" value="<%= edit ? e.getDescripcion() : "" %>" required>
        </div>
        ...
        <div>
          <label>Lugar</label>
          <input class="input" name="lugar" value="<%= edit ? e.getLugar() : "" %>" required>
        </div>

        <div>
          <label>Aforo</label>
          <input class="input" type="number" name="aforo" value="<%= edit ? e.getAforo() : "" %>" required>
        </div>

        <div>
          <label>Fecha Inicio</label>
          <input class="input" type="datetime-local" name="fechaInicio" value="<%= fechaInicio %>" required>
        </div>

        <div>
          <label>Fecha Fin</label>
          <input class="input" type="datetime-local" name="fechaFin" value="<%= fechaFin %>" required>
        </div>
